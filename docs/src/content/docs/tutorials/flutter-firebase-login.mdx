...
- `AppLogoutPressed` which notifies the bloc that the current user has requested to be logged out.
...
In the `_onUserSubscriptionRequested` event handler, the `AppBloc` uses `emit.onEach` to subscribe to the user stream of the `AuthenticationRepository` and emit a state in response to each emitted `User`.
